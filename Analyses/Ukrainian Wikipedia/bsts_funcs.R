...
make_model <- function(data,
  ...
  if (dynamic_coefficient) {
    # Can't get this to work! "initial value leads to infinite probability in ScalarSliceSampler" :(
    ss <- AddDynamicRegression(ss, `Ukrainian Wikipedia (Main Page) PVs from Portal's Russian-but-not-Ukrainian-speaking visitors` ~ post_deployment, data = ukwiki)
  }
  # Prior:
...
make_model_3 <- function(data,
  ...
  # Aggegated Data:
  ukwiki <- data[source_speaks == "ukrainian wikipedia (main page) from Wikipedia.org Portal (from users who speak Ukrainian)" | source %in% control_markets,
                 list(pageviews = sum(pageviews), post_deployment = max(post_deployment_indicator)),
                 by = c("date", "source")] %>% make_data_bsts_ready("ukrainian wikipedia (main page) from Wikipedia.org Portal")
  y <- ukwiki$`ukrainian wikipedia (main page) from Wikipedia.org Portal`
  # State Specification:
...
make_model_4 <- function(data,
  ...
  # Aggegated Data:
  ukwiki <- data[source_speaks == "ukrainian wikipedia (main page) from Wikipedia.org Portal (from users who speak Ukrainian and Russian)" | source %in% control_markets,
                 list(pageviews = sum(pageviews), post_deployment = max(post_deployment_indicator)),
                 by = c("date", "source")] %>% make_data_bsts_ready("ukrainian wikipedia (main page) from Wikipedia.org Portal")
  y <- ukwiki$`ukrainian wikipedia (main page) from Wikipedia.org Portal`
  # State Specification:
